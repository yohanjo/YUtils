...
public class HSTTM8W implements Callable<Void> {
    ...
    static boolean tokenization;  // Do tokenization?
    ...
    static DoubleMatrix P_BW;  // Pre-trained background topic x word probabilities
    ...
    static TreeMap<Integer, DoubleMatrix> P_DB = new TreeMap<>();  // Domain x background topic dist
    ...
    // Domains (i.e., Wikipedia content page titles)
    ...
    public static class Parameters {
        ...
        @Parameter(names = "-background", description = "Trained background path.")
    ...
    public static void printConfiguration() {
        ...
        System.out.println("Background topics: "+numBTopics);
     ...
     */
    public HSTTM8W(int threadId, List<String> seqIds) {
    ...
    public static void runSampling() throws Exception {
        ...
        for (int iter = 0; iter < numIters; iter++) {
            ...
            for (int tid = 0; tid < numThreads; tid++) {
                tasks.add(new HSTTM8W(tid, seqIds));
            }
    ...
    public static void indexizeInstances() {
        for (String seqId : rawSeqs.keySet()) {
            ...
            if (seq.instances.size() >= minSeqLen) {
                seq.domain = domainIndex.get(seqDomain.get(seqId));
                seqs.put(seqId, seq);
            ...
            for (int d = 0; d < insts.size(); d++) {
                ...
                for (Sentence sentence : inst.sentences) {
                    ...
                    for (Word word : sentence.words) {
                        word.bTopic = random.nextInt(numBTopics);
    ...
    public void sampleBTopic(List<String> seqIds) throws Exception {
        for (String seqId : seqIds) {
            Sequence seq = seqs.get(seqId);
            DoubleMatrix P_B = P_DB.get(seq.domain);
            
            for (Instance inst : seqs.get(seqId).instances.values()) {
                for (Sentence sentence : inst.sentences) {
                    for (Word word : sentence.words) {

                        // Calculate the conditional probability of each topic being assigned to this sentence
                        double [] probs = new double[numBTopics];
                        for (int t = 0; t < numBTopics; t++) {
                            probs[t] += Math.log(P_B.getValue(0,t));
                            if (word.level==0) {
                                if (P_BW.getValue(t,word.id)==0) probs[t] = Double.MIN_VALUE; 
                                else probs[t] += Math.log(P_BW.getValue(t,word.id));
                            }
                        }

                        double maxLogP = max(probs);
                        for (int t = 0; t < numBTopics; t++) {
                            if (Double.isNaN(probs[t])) probs[t] = 0;
                            else probs[t] = Math.exp(probs[t] - maxLogP);
                        }
                        
                        int newTopic = sampleIndex(probs);
                        word.bTopic = newTopic;
                    }
                }
            }
        }
            ...
            for (Instance inst : seqs.get(seqId).instances.values()) {
                for (Sentence sentence : inst.sentences) {
                    ...
                    for (Word word : sentence.words) {
                        ...
                        int bTopic = word.bTopic;
                        int w = word.id;
                        
                        if (updateParams && oldLevel == 1)
                            V_FW.decValue(fTopic, w);
                        
                        double [] probs = new double[2];
                        probs[0] = (1-eta) * P_BW.getValue(bTopic,w);
                        ...
                        if (updateParams && newLevel == 1) 
                            V_FW.incValue(fTopic, w);
                    }
                }
            }
        }
    ...
    public static void genOutFiles(int iter) throws Exception {
        ...
        // ThetaB
        CSVPrinter outBTheta = new CSVPrinter(new PrintFileWriter(outDir+"/"+outPrefix+"-I"+iter+"-ThetaB.csv"), CSVFormat.EXCEL);
        outBTheta.print("SeqId");
        for (int t = 0; t < numBTopics; t++) outBTheta.print("BT"+t);
        outBTheta.println();
        
        for (String seqId : seqs.keySet()) {
            DoubleMatrix P_B = P_DB.get(seqs.get(seqId).domain);
            outBTheta.print(seqId);
            for (int t = 0; t < numBTopics; t++)
                outBTheta.print(P_B.getValue(0,t));
            outBTheta.println();    
        }
        outBTheta.flush();
        outBTheta.close();
        ...
        for (int w = 0; w < numWords; w++) {
            ...
            for (int t = 0; t < numBTopics; t++)
                outPhiB.print(P_BW.getValue(t,w));
            outPhiB.println();
        }
        outPhiB.flush();
        outPhiB.close();
        ...
        IntegerMatrix I_BW = P_BW.getSortedIndexMatrix(1, Math.min(numProbWords, numWords));
        for (int p = 0; p < Math.min(numProbWords, numWords); p++) {
            for (int t = 0; t < numBTopics; t++) {
                int w = I_BW.getValue(t,p);
                outProbWordsB.print(String.format("%s (%.3f)", wordList.get(w), 
                                                                    P_BW.getValue(t,w)));
            }
            outProbWordsB.println();
        }
        ...
        outInstAssign.printRecord("SeqId", "InstNo", "Author", "Text", "TaggedText", "State");
                    ...
                    for (Sentence sentence : inst.sentences) {
                        for (Word word : sentence.words) {
                            if (word.level==1) taggedText.append(
                                    ...
                                    "B"+word.bTopic+":" + wordList.get(word.id) + " ");
                        }
                    }
        ...
        outSentAssign.printRecord("SeqId", "InstNo", "Author", "Sentence", "TaggedText", 
                                  "State", "FTopic");
    ...
    public static double logLikelihood() {
        ...
        for (String seqId : seqs.keySet()) {
            ...
            for (int r = 0; r < P_AS.getNumRows(); r++) {
                ...
                for (int c = 0; c < P_AS.getNumColumns(); c++)
            ...
            for (int d = 0; d < seq.size(); d++) {
                ...
                // State transition
                if (inst.parent == null) logP += Math.log(nu * P_0S.getValue(0, state) + (1-nu) * P_AS.getValue(inst.author, state));
                else logP += Math.log(nu * P_SS.getValue(inst.parent.state, state) + (1-nu) * P_AS.getValue(inst.author, state));
                ...
                for (Sentence sentence : inst.sentences) {
                    ...
                    for (Word word : sentence.words) {
                        // BTopic
                        logP += Math.log(P_B.getValue(0, word.bTopic));

                        if (word.level == 1)
                            logP += Math.log(eta) + Math.log(P_FW.getValue(sentence.fTopic, word.id));
                        else
                            logP += Math.log(1-eta) + Math.log(P_BW.getValue(word.bTopic, word.id));
                    }
                }
            }
        }
    ...
    /**
     * Loads the topic-word matrix {@link #P_BW} and article-topic matrix 
     * {@link #P_DB} of the content pages trained by LDA.
     */
    public static void loadBackground() throws Exception {
        // Load background topics
        CSVParser bgPhi = new CSVParser(new FileReader(backgroundPathPrefix+"-Phi.csv"),
                                        CSVFormat.EXCEL.withHeader());
        numBTopics = bgPhi.getHeaderMap().size() - 1;
        P_BW = new DoubleMatrix(numBTopics, numWords); 
        for (CSVRecord record : bgPhi) {
            Integer w = wordIndex.get(record.get(0));
            if (w == null) continue;  // Ignore words not in this data
            for (int t = 0; t < numBTopics; t++)
                P_BW.setValue(t,w, Double.valueOf(record.get(t+1)));
        }
        bgPhi.close();
        
        // Load background theta
        CSVParser bgTheta = new CSVParser(new FileReader(backgroundPathPrefix+"-Theta.csv"),
                                                        CSVFormat.EXCEL.withHeader());
        for (CSVRecord record : bgTheta) {
            if (!domainIndex.containsKey(record.get(0))) continue;
            DoubleMatrix P_B = new DoubleMatrix(1, numBTopics);
            for (int t = 0; t < numBTopics; t++)
                P_B.setValue(0, t, Double.valueOf(record.get(t+1)));
            P_DB.put(domainIndex.get(record.get(0)), P_B);
        }
        bgTheta.close();
    }
